...
namespace MPLVS {
  ...
  public class MplLanguage : LanguageService {
    ...
    public override int ValidateBreakpointLocation(IVsTextBuffer buffer, int line, int col, TextSpan[] pCodeSpan) {
      ...
      return AjastBreakpoint(buffer, pCodeSpan);
    }

    private static int AjastBreakpoint(IVsTextBuffer buffer, TextSpan[] pCodeSpan) {
      ...
      pCodeSpan[0] = AsTextSpun(token, lines);

      return VSConstants.S_OK;
    }

    public static TextSpan AsTextSpun(Builder.Node node, ITextBuffer buffer) {
